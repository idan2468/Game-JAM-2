...
public class ObjectSpawner : MonoBehaviour
{
    ...
    private Queue<TempEnemyControler> _enemiesPool;
    private Queue<TempJewControler> _jewsPool;
    ...
    void Start()
    {
        ...
        _instance = this;
        _enemiesPool = new Queue<TempEnemyControler>();
        _jewsPool = new Queue<TempJewControler>();
    ...
    private void InitializeQueue(int numOfObject, ObjectType objNameToInitialize)
    {
        ...
        for (int i = 0; i < numOfObject; i++)
        {
            ...
            {
                _jewsPool.Enqueue(obj.GetComponent<TempJewControler>());
            }
            else
            {
                _enemiesPool.Enqueue(obj.GetComponent<TempEnemyControler>());
            }
        }
    }
    ...
    public TempJewControler SpawnJew()
    {
    ...
    public TempEnemyControler SpawnEnemy()
    {
    ...
    public void RemoveObject(TempJewControler jew)
    {
        jew.gameObject.SetActive(false);
        jew.transform.SetParent(jewsPoolContainer);
        _jewsPool.Enqueue(jew);
    }
    public void RemoveObject(TempEnemyControler enemy)
    {
